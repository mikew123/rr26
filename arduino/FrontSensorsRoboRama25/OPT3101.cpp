...
void OPT3101::resetAndWait()
{
  ...
  Wire1.write(0);
  Wire1.write(1);
  lastError = Wire1.endTransmission();
  if (lastError) { return; }
...
void OPT3101::writeReg(uint8_t reg, uint32_t value)
{
  Wire1.beginTransmission(address);
  Wire1.write(reg);
  Wire1.write((uint8_t)(value >> 0));
  Wire1.write((uint8_t)(value >> 8));
  Wire1.write((uint8_t)(value >> 16));
  lastError = Wire1.endTransmission();
}
...
uint32_t OPT3101::readReg(uint8_t reg)
{
  Wire1.beginTransmission(address);
  Wire1.write(reg);
  lastError = Wire1.endTransmission(false); // no stop (repeated start)
  if (lastError)
  {
    return 0;
  }

  uint8_t byteCount = Wire1.requestFrom(address, (uint8_t)3);
  if (byteCount != 3)
  {
    lastError = 50;
    return 0;
