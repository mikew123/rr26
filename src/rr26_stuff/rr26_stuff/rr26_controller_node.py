...
MRW 3/25/2026 converted to non-lifecycle
"""
...
from .rr26_barrel_race import BarrelRace

class Roborama25ControllerNode(Node):
    """
    ...
    def __init__(self, nav: BasicNavigator):
        super().__init__('rr26_controller_node')
        ...
        self.map_msg_publisher = self.create_publisher(OccupancyGrid, 'map', qos_profile=qos_profile)

        self.map_timer = self.create_timer(0.1, self.nav2_run, callback_group=self.cb_group_mx)

        self.barrelRace = BarrelRace(self)
        # self.barrelRace.runBarrelRace()
    ...
    def cleanup(self) :
        ...
        self.map_msg_publisher.destroy()
    ...
    def nav2_run(self) :
        ...
        if self.gotoCan==True and self.gotoCan_last==False :
            ...
            self.runBarrelRace() # Blocking call on Button Y
    ...
    def runBarrelRace(self) :
        """
        Button Y - starts Barrel Race
        The fuction is blocking and returns when barrel race is complete
        """
        ...
        # calls a blocking function, returns when complete
        self.barrelRace.runBarrelRace()
    ...
    def barrels_callback(self, msg:Barrels) -> None:
        """
        Call code in barrel_race.py
        """
        self.barrelRace.barrels_callback(msg)
    ...
    def driveOdom(self, dist: float, vx: float=0.5) :
        ...
        while rclpy.ok() and elapsed_time <(t-dt) :
            self.cmd_vel_publisher.publish(msg)
            time.sleep(dt)
...
#        self.send_set_param_request(self.global_costmap_set_param_svc, 'obstacle_layer.enabled', False)
        ...
        # disable acceleration limit
        cmd_json = {"wheels": {"accel": 100.0}}
...
def main(args=None):
    ...
    try:
        nav = BasicNavigator()
        node = Roborama25ControllerNode(nav)
